...
void UltralightController::OnDOMReady(ultralight::View *caller, uint64_t frame_id, bool is_main_frame,
...
void UltralightController::OnBeginLoading(ultralight::View *caller, uint64_t frame_id, bool is_main_frame,
                                          const ultralight::String &url)
{
}
...
void UltralightController::OnFinishLoading(ultralight::View *caller, uint64_t frame_id, bool is_main_frame,
                                           const ultralight::String &url)
{
    if (is_main_frame)
    {
...
void UltralightController::OnFailLoading(ultralight::View *caller, uint64_t frame_id, bool is_main_frame,
...
std::string UltralightController::jsValueToErrorString(JSValueRef err)
{
    ...
    if (!ctx)
        return "Could not get JS context for error reporting.";
...
{
    JSContextRef ctx = getJSContext();
    if (!ctx || !val || !JSValueIsString(ctx, val))
        return "";
...
JSValueRef UltralightController::callFunction(JSObjectRef func, JSObjectRef thisObj,
                                              const std::vector<JSValueRef> &args, std::string &err)
{
    JSContextRef ctx = getJSContext();
    if (!ctx || !func)
    {
...
JSObjectRef UltralightController::createTypedArrayWithBytes(JSTypedArrayType arrayType, void *bytes, size_t byteLength,
                                                            std::string &err)
{
    JSContextRef ctx = getJSContext();
    if (!ctx)
    ...
    JSObjectRef result =
        JSObjectMakeTypedArrayWithBytesNoCopy(ctx, arrayType, bytes, byteLength, nullptr, nullptr, &exception);
    if (exception)
    {
        err = jsValueToErrorString(exception);
    }
